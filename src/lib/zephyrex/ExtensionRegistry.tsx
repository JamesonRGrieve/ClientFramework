...
export function useActiveExtensions(
  registeredExtensions: ZephyrexClientExtension[],
): { active: ZephyrexClientExtension[]; loading: boolean } {
  ...
  const active = useMemo(() => {
    ...
    return registeredExtensions.filter(
      (ext) => !ext.serverExtension || serverNames.has(ext.serverExtension),
    );
  ...
  return (
    <div className="p-4">
      ...
      <p className="text-sm text-muted-foreground">
        Configure {extensionName} via the server API settings endpoint.
      </p>
    </div>
  );
}

...
>(({ side = 'left', variant = 'sidebar', collapsible = 'offcanvas', className, style, children, ...props }, ref) => {
  const { isMobile, state, openMobile, setOpenMobile, width } = useSidebar(side);

  const sidebarStyle = React.useMemo(
    () => ({
      '--sidebar-width': state === 'expanded' ? `${width}px` : SIDEBAR_WIDTH_ICON,
      ...style,
    }),
    [state, width, style],
  ) as React.CSSProperties;
...
>(({ side = 'left', minWidth = 200, maxWidth = 600, className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        'absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex hover:cursor-pointer',
        'in-data-[side=left]:cursor-w-resize in-data-[side=right]:cursor-e-resize',
        '[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize',
        'group-data-[collapsible=offcanvas]:translate-x-0 group-data-[collapsible=offcanvas]:after:left-full hover:group-data-[collapsible=offcanvas]:bg-sidebar',
        '[[data-side=left][data-collapsible=offcanvas]_&]:-right-2',
        '[[data-side=right][data-collapsible=offcanvas]_&]:-left-2',
        'data-[resizing=true]:after:bg-sidebar-border',
        className,
      )}
      {...props}
    />
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'group-data-[collapsible=icon]:group-data-[side=right]:p-0 group-data-[collapsible=icon]:group-data-[side=right]:gap-0',
        className,
      )}
      {...props}
    />

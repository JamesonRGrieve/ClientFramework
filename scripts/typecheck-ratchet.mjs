 ...
 * Type-check ratchet.
 *
 * Runs `tsc --noEmit` and compares the error count to `.tsc-error-baseline`.
 * Fails when the count goes UP, succeeds when it goes down or stays the same.
 *
 *   node scripts/typecheck-ratchet.mjs            # check
 *   node scripts/typecheck-ratchet.mjs --update   # rewrite baseline to current count
 *
 * Adapted from JamesonRGrieve/wh40k-ttrpg-foundry-vtt:scripts/typecheck-ratchet.mjs.
 */
...
if (current > baseline) {
  ...
  console.error('Fix the new errors, or if intentional run: pnpm run typecheck:ratchet:update');
  process.exit(1);
}

if (current < baseline) {
  console.log(`[typecheck-ratchet] OK: tsc errors decreased ${baseline} -> ${current} (-${baseline - current}).`);
  console.log('Lower the baseline in the same commit: pnpm run typecheck:ratchet:update');
  process.exit(0);
}

#!/usr/bin/env node
/**
 * Per-rule, per-directory TS-strictness ratchet. Runs the same scan as
 * scripts/ts-coverage.mjs and compares the result to `.ts-coverage-baseline`.
 * Fails when ANY (metric, directory) pair regresses upward. New directories
 * are added at their current count; removed directories are dropped.
 *
 *   node scripts/ts-ratchet.mjs            # check
 *   node scripts/ts-ratchet.mjs --update   # rewrite baseline to current counts
 *
 * Independent of typecheck-ratchet (total tsc errors). This gates the four
 * manual-suppression patterns most likely to be introduced when fixing types
 * under time pressure.
 *
 * Adapted from JamesonRGrieve/wh40k-ttrpg-foundry-vtt:scripts/ts-ratchet.mjs.
 */
...
function* walk(dir) {
  ...
  for (const name of readdirSync(dir)) {
    ...
    else if (stat.isFile() && isCountable(name)) yield full;
  }
}
...
if (failures.length) {
  ...
  console.error('Fix the new occurrences, or if intentional run: pnpm run ts:ratchet:update');
  process.exit(1);
}
...
if (improved) {
  console.log('[ts-ratchet] OK: counts decreased. Lower the baseline in the same commit: pnpm run ts:ratchet:update');
} else {

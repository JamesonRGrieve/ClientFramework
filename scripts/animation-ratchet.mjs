...
if (current > baseline) {
  console.error(
    `[animation-ratchet] FAIL: animation usages rose ${baseline} -> ${current} (+${current - baseline}). See ${COVERAGE}.`,
  );
  console.error(
    'Prefer Tailwind v4 @theme tokens or tailwindcss-animate utilities over hand-rolled keyframes/declarations.',
  );
  process.exit(1);
}
...
if (current < baseline) {
  console.log(
    `[animation-ratchet] OK: animation usages dropped ${baseline} -> ${current}. Lower baseline: pnpm animation:ratchet:update`,
  );
  process.exit(0);
}

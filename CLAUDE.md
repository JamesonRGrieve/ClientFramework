...
`auth` and `zod2gql` previously lived as git submodules in `src/components/auth/` and `src/lib/zod2gql/`. Both are being repackaged as published npm dependencies and consumed via `package.json` rather than path aliases, so framework merges are not blocked on cross-repo submodule pointer churn and downstream forks can pin a specific version (or swap `auth` for Auth0/Clerk wholesale) without editing framework files.

- Until the npm packages are published, imports referencing `@/auth/*` or `@/zod2gql` will fail. Update those import sites to the new package names as part of the same PR that bumps the dependency.
- The other previously-split modules (`appwrapper`, `dynamic-form`, `next-log`) remain inlined under `src/components/` and `src/lib/` because they were heavily coupled to framework internals and never published as standalone packages; the submodule overhead exceeded the unrealized reuse benefit.
